...
export type SoundPreset = 'gov-standard' | 'urgent-alert' | 'crystal-ding' | 'modern-beep' | 'soft-chime';
...
export interface SoundSettings {
  ...
  preset: SoundPreset;
  playOnUrgentOnly: boolean;
  playOnNewMessage: boolean;
  playOnAutoResolved: boolean;
}
...
export interface AppNotification {
  ...
  message: string;
  time: string;
  type: 'urgent' | 'message' | 'system' | 'bot' | 'sla';
  isRead: boolean;
  targetTab?: TabKey;
  ticketId?: string;
}
...
export interface MessageItem {
  id: string;
  sender: 'citizen' | 'officer' | 'ai_bot' | 'system';
  senderName?: string;
  text: string;
  time: string;
...
export interface AfterHoursRule {
  id: string;
  title: string;
  scheduleType: 'night_shift' | 'weekend' | 'holiday' | 'all_officers_busy';
...
export interface NightShiftLog {
  id: string;
  citizenName: string;
  phone: string;
  channel: ChannelType;
  ...
  actionTaken: 'ai_instant_answered' | 'scheduled_morning_callback' | 'escalated_oncall_officer';
  ticketCode: string;
  callbackAssignedTo: string;
  callbackTimeTarget: string;
  status: 'pending_morning' | 'completed' | 'notified';
}
...
export interface BroadcastReply {
  ...
  replyText: string;
  receivedAt: string;
  status: 'new' | 'routed_to_inbox' | 'replied';
  assignedOfficer?: string;
}
